...
By applying a variational (weak) formulation, the governing PDE is transformed into a global system of algebraic equations. For a wide class of physics problems, this process leads to a sparse linear system of the form $\mathbf{K}\mathbf{u} = \mathbf{f}$, where $\mathbf{K}$ is the global stiffness matrix, $\mathbf{u}$ is the vector of nodal unknowns, and $\mathbf{f}$ represents the global load vector. The stiffness matrix is typically sparse, symmetric, and positive definite, properties that strongly influence the choice of numerical solvers and optimisation strategies.
...
Within the scope of this project, the initial focus is placed on particular set of sub-problems governed by Poisson's Equation, which provide a mathematically well-posed and computationally controlled setting for evaluating parallel execution strategies. These formulations are representative of several physical phenomena and allow the solver architecture to be extended to related problems with minimal implementation changes.
...
To evaluate the proposed solver, a set of two-dimensional problems governed by the Laplace equation will be adopted as a benchmark. These problems describe steady-state phenomena in which the scalar field is harmonic within the domain, meaning that no internal sources or sinks are present. Such formulations are commonly encountered in heat conduction, diffusion, electrostatics, and potential flow, making them well suited for assessing both numerical accuracy and computational performance.

The governing equation is given by $\nabla^2 \phi = 0$ given a certain $\Omega$, where $\phi$ represents the scalar field of interest and $\Omega$ denotes the two-dimensional computational domain.

Such formulations arise naturally in potential flow, heat conduction, diffusion, and electrostatics, and are well suited for FEM discretization and performance analysis. The differences between problem implementations reside in the physical meaning of the scalar variable and the boundary conditions applied. The following table illustrates how these problems are related:

| Phenomenon        | Scalar Variable        | Associated Flux   |
| ----------------- | ---------------------- | ----------------- |
| Heat Conduction   | Temperature            | Heat Flux         |
| Mass Diffusion    | Concentration          | Mass Flux         |
| Electrostatics    | Electric Potential     | Electric Field    |
| Potential Flow    | Velocity Potential     | Velocity          |

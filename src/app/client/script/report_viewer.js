...
export class ReportViewerPanel {
    ...
    initializeCharts() {
        ...
        // Use shared ChartUtils if available
        if (window.ChartUtils) {
            this.chartInstances = window.ChartUtils.initializeCharts(container);
        } else {
            console.log('[ReportViewer] ChartUtils not available, charts will not render');
        }
    }
    ...
    close() {
        ...
        // Dispose ECharts using shared utility
        if (this.panel && window.ChartUtils) {
            const container = this.panel.querySelector('.report-viewer-content');
            if (container) {
                window.ChartUtils.disposeCharts(container);
            }
        }
        this.chartInstances = [];
